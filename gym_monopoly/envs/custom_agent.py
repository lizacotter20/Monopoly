...
def make_out_of_turn_move(player, current_gameboard, allowable_moves, code):
    ...
    if player.status != 'current_move': # these actions are considered only if it's NOT our turn to roll the dice.
        ...
        if player.current_cash < current_gameboard['go_increment'] and player.current_cash <= 150 and action_choices.make_sell_property_offer in allowable_moves: # let's try to see if we can sell property to other players at a premium
...
def make_buy_property_decision(player, current_gameboard, asset):
    ...
    if player.current_cash - asset.price >= current_gameboard['go_increment']:  # case 1: can we afford it?
        ...
        decision = True
    elif asset.price <= player.current_cash and player.current_cash >= 350: #and agent_helper_functions.will_property_complete_set(player,asset,current_gameboard):
        print(player.player_name, ': I will attempt to buy ', asset.name, ' from the bank.')
        decision = True
