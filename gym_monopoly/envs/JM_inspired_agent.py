...
def make_post_roll_move(player, current_gameboard, allowable_moves, code):
    ...
    action_choices
        """
    """The "good" properties are the ones that have <30 move break even with a hotel """
...
def make_buy_property_decision(player, current_gameboard, asset):
    ...
    """
    goodProp = ["Boardwalk", "New York Ave", "Illinois Ave", "Tennessee Ave", "St. James Place"]
    railroads = ["B&O Railroad", "Short Line", "Pennsylvania Railroad", "Reading Railroad"]
    utilities = ["Water Works", "Electric Company"]
    params = dict()
        params['player'] = player
        params['asset'] = current_gameboard['location_sequence'][player.current_position]
    #good, railR, util = False
    if current_gameboard['num_die_rolls'] < 10
        if player.current_cash >= asset.price:
            decision = True
    if current_gameboard['num_die_rolls'] > 10 && current_gameboard['num_die_rolls'] < 20 
        for x in goodProp
            if asset.name == railroads[x] && current_gameboard['num']
                decision = True
            if asset.name == util[x] 
                decision = True

                if asset.name == goodProp[x] 
                good = True
